...
export const createFhevmInstance = async (parameters: {
  ...
  if (isMock) {
    ...
    if (fhevmRelayerMetadata) {
      // In non-development builds, require explicit opt-in to use mock
      try {
        // process.env is replaced at build-time by Next.js
        const nodeEnv = process.env.NODE_ENV;
        const enableMock = process.env.NEXT_PUBLIC_ENABLE_FHEVM_MOCK;
        if (nodeEnv !== "development" && enableMock !== "1") {
          throwFhevmError(
            "FHEVM_MOCK_DISABLED",
            "FHEVM mock mode is disabled outside development. Set NEXT_PUBLIC_ENABLE_FHEVM_MOCK=1 to enable it explicitly."
          );
        }
      } catch {
        // ignore env read issues, continue
      }
